...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Nouvel Acte du Chef de l'État</h1>
            <p class="text-gray-600 dark:text-gray-400">Créez un nouvel acte du président</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
        <form action="{{ route('admin.social-links.store') }}" method="POST" class="space-y-6 p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="act_type" id="act_type" 
                        ...
                        <option value="decret" {{ old('act_type') == 'decret' ? 'selected' : '' }}>Décret</option>
                        <option value="arrete" {{ old('act_type') == 'arrete' ? 'selected' : '' }}>Arrêté</option>
                        <option value="ordonnance" {{ old('act_type') == 'ordonnance' ? 'selected' : '' }}>Ordonnance</option>
                        <option value="decision" {{ old('act_type') == 'decision' ? 'selected' : '' }}>Décision</option>
                        <option value="circulaire" {{ old('act_type') == 'circulaire' ? 'selected' : '' }}>Circulaire</option>
                        <option value="autre" {{ old('act_type') == 'autre' ? 'selected' : '' }}>Autre</option>
                    </select>
                    @error('act_type')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="act_number" id="act_number" 
                           class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-700 dark:text-white"
                           value="{{ old('act_number') }}" placeholder="Ex: 2024-001">
                    @error('act_number')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="signature_date" id="signature_date" 
                           class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-700 dark:text-white"
                           value="{{ old('signature_date') }}" required>
                ...
                <div>
                    ...
                    <input type="text" name="signature_location" id="signature_location" 
                           class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-700 dark:text-white"
                           value="{{ old('signature_location') }}" placeholder="Ex: Palais de la Renaissance, Bangui" required>
                ...
                <div>
                    ...
                    <input type="url" name="document_url" id="document_url" 
                           class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-700 dark:text-white"
                           value="{{ old('document_url') }}" placeholder="https://example.com/document.pdf">
                    @error('document_url')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
