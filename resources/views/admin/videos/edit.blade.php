...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __('admin.edit_video') }}</h1>
        <div class="flex space-x-3">
            <a href="{{ route('admin.videos.show', $video) }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-200">
                <i class="fas fa-eye mr-2"></i>{{ __('admin.view') }}
            </a>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                                  placeholder="{{ __('admin.video_description_placeholder') }}">{{ old('description', $video->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div id="url-content" class="tab-content">
                            ...
                            @if($video->video_url)
                                <div class="mt-2">
                                    <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('admin.current_video') }}:</p>
                                    <a href="{{ $video->video_url }}" target="_blank" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-sm">
                    ...
                    <div>
                        ...
                        <div id="thumb-url-content" class="thumb-tab-content">
                            ...
                            @if($video->thumbnail_url)
                                <div class="mt-2">
                                    <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('admin.current_thumbnail') }}:</p>
                                    <img src="{{ $video->thumbnail_url }}" alt="{{ __('admin.current_thumbnail') }}" class="w-32 h-20 object-cover rounded mt-1">
                                </div>
                            @endif
                            @error('thumbnail_url')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" id="featured" name="featured" value="1" 
                                   {{ old('featured', $video->is_featured) ? 'checked' : '' }}
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                        <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-2">{{ __('admin.current_information') }}</h4>
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">{{ __('admin.created_at') }}:</span>
                                <span class="text-gray-900 dark:text-white">{{ $video->created_at->format('d/m/Y H:i') }}</span>
                            </div>
                            @if($video->updated_at != $video->created_at)
                                <div class="flex justify-between">
                                    <span class="text-gray-600 dark:text-gray-400">{{ __('admin.updated_at') }}:</span>
                                    <span class="text-gray-900 dark:text-white">{{ $video->updated_at->format('d/m/Y H:i') }}</span>
                                </div>
                            @endif
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">{{ __('admin.status') }}:</span>
                                <span class="text-gray-900 dark:text-white">{{ $video->is_featured ? __('admin.featured') : __('admin.normal') }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-200">
                            <i class="fas fa-save mr-2"></i>{{ __('admin.update_video') }}
                        </button>
